...
class attack_msf():
    ...
    def rendering_img(self, xxx):
        ...
        with torch.no_grad():
            ...
            length = self.length
            i_final, vertex, vertex_orig, face, pytorchModel = self.i_final.cuda(), self.object_v.cuda(), l_c_c_ori.cuda(), self.object_f.cuda(), self.LiDAR_model.cuda()
            point_cloud = render.render(ray_direction, length, vertex, face,
                                                       i_final)
            grid_hard = xyzi2gridhard_2(point_cloud[:, 0], point_cloud[:, 1], point_cloud[:, 2], point_cloud[:, 3])
