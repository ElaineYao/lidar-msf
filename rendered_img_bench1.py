...
class attack_msf():
    ...
    # x_of=7
    def load_mesh(self, path, r, x_of=40, y_of=8):
        # z_of = -1.73 + r / 2.
        # z_of = -1.73 + r / 2.
        global xof
        global yof
        ...
        # r is scaling factor controlling the size of the 3d object. I think this will influence the detection accuracy 
        ...
        # blue 30,144,255
        # self.object_t[:, :, :, :, 0] = 30/255
        # self.object_t[:, :, :, :, 1] = 144/255
        # self.object_t[:, :, :, :, 2] = 255/255
    ...
    def parse_yolo_region(self, outputs):
        # ===========================
        global xof
        global yof
        global rof
        objects = []
        for layer_name, out_blob in enumerate(outputs):
            print("##########")
            ...
            # print("Layer:{}\nOutBlob:{}".format(layer_name, out_blob.shape))
          
            for oth in range(0, out_blob.shape[1], 85):   # 255->out_blob.shape[1] 85*3 = 255, 3 anchors
                for row in range(out_blob.shape[2]):       # 13 represent the grid
                    for col in range(out_blob.shape[3]):   # 13
                        info_per_anchor = out_blob[0, oth:oth+85, row, col] #print("prob"+str(prob))
                        x, y, width, height, prob = info_per_anchor[:5]
                        # cfs = torch.nn.functional.sigmoid(out[:, 4]).cuda()
                        threshold = 0.5
                        iou_threshold = 0.01
                        prob = torch.nn.functional.sigmoid(prob).cuda()
                        ...
                        info_per_anchor = info_per_anchor.detach()
                        scores = info_per_anchor[5:]
                        class_id = np.argmax(scores)
                        score = torch.nn.functional.sigmoid(scores[class_id]).cuda()
                        prob_score = prob*score  # score for each class
                        # print(str(class_id)+" prob: "+str(prob*score))
                        # use if the class_id == 56 or 13 as metric
                        # bounding boxes
                        params_side = row
                        x = (col + x) / params_side
                        y = (row + y) / params_side
                        try:
                            ...
                            continue
                        params_isYoloV3 = True
                        params_anchors = [10,13,  16,30,  33,23,  30,61,  62,45,  59,119,  116,90,  156,198,  373,326]
                        resized_image_w = 416
                        resized_image_h = 416
                        size_normalizer = (resized_image_w, resized_image_h) if params_isYoloV3 else (params_side, params_side)
                        n = int(oth/85)
                        width = width * params_anchors[3 * n] / size_normalizer[0]
                        height = height * params_anchors[3 * n + 1] / size_normalizer[1]
                        w_scale=416
                        h_scale=416
                        xmin = int((x - width / 2) * w_scale)
                        ymin = int((y - height / 2) * h_scale)
                        xmax = int(xmin + width * w_scale)
                        ymax = int(ymin + height * h_scale)
                        object = {"xmin":xmin, "xmax":xmax, "ymin":ymin, "ymax":ymax, "class_id": class_id.detach(), "confidence": prob_score.detach()}
                        ...
                        for i in range(len(objects)):
                            ...
                            for j in range(i + 1, len(objects)):
                                ...
                                # print("********")
                                # print(objects[i]['class_id'])
                                # print(objects[j]['class_id'])
                                # print(objects[i]['class_id'] == objects[j]['class_id'])
                                if(objects[i]['class_id'] != objects[j]['class_id']): continue
                                print(self.intersection_over_union(objects[i], objects[j]))
                        ...
                        # print("xmin: "+str(xmin)+", xmax: "+str(xmax)+", ymin: "+str(ymin)+", ymax: "+str(ymax)+", classid: "+str(class_id)+", confidnece: "+str(prob_score))
        # print(objects)
        print(len(objects))
        ...
        with open('./output1.csv','a', newline='') as csvfile:
            ...
            csvWriter.writerow([len(objects)])
            csvWriter.writerow([xof, yof, rof])
            for i in range(len(objects)):
                item = objects[i]
                csvWriter.writerow([item['class_id'].cpu().numpy(), item['confidence'].cpu().numpy() ])
    ...
    def rendering_img(self, ppath):
        ...
        imsave('./result/bench1/benign.jpg', image)
        final, outputs = self.model(final_image)
        self.parse_yolo_region(outputs)
